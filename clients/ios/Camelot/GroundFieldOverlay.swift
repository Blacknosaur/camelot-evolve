...
enum GroundFieldOverlay {
    ...
    static func path(calibration: GroundCalibration, frame: CGRect) -> CGPath {
        ...
        for polyline in worldLines(reference: reference, length: calibration.lengthMeters, depth: calibration.widthMeters) {
            // A long straight line with one end past the horizon must still
            // show its visible part: sample it before projecting.
            var line: [CGPoint] = []
            for (a, b) in zip(polyline, polyline.dropFirst()) {
                let steps = max(1, Int(hypot(b.x - a.x, b.y - a.y) / 2))
                for step in 0..<steps {
                    let t = Double(step) / Double(steps)
                    line.append(CGPoint(x: a.x + (b.x - a.x) * t, y: a.y + (b.y - a.y) * t))
                }
            }
            if let last = polyline.last { line.append(last) }
