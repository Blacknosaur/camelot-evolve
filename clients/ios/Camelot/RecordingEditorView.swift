...
struct RecordingEditorView: View {
    ...
    init(recording: Recording, startsInClips: Bool = false, initialClips: [CompositionClip]? = nil, composition: VideoComposition? = nil) {
    ...
    var body: some View {
        NavigationStack {
            AdaptiveLayout { layout in
                ZStack {
                    ...
                    if layout.isLandscape {
                        let sidebar = min(max(300, preferredSidebarWidth), max(300, layout.size.width - 240))
                        HStack(spacing: 0) {
                            videoCanvas.frame(maxWidth: .infinity, maxHeight: .infinity)
                            EditorPanelDivider(title: "Workspace", vertical: true, value: sidebar, limits: 300...max(300, layout.size.width - 240)) { preferredSidebarWidth = $0 }
                            editorWorkspace(height: layout.size.height).frame(width: sidebar)
                        }
                    } else {
            ...
            .toolbarBackground(Color.editorBackground, for: .navigationBar)
            ...
            .sheet(isPresented: $showingDetails, onDismiss: { if deletedVideo { dismiss() } }) {
            ...
            .fullScreenCover(item: $analysisWorkspace) { request in
                AnalysisWorkspaceView(request: request, session: analysisSession) { edited in
                    try saveAnalysis(edited, request: request)
                }
            }
    ...
    private func openAnalysis(_ mode: AnalysisWorkspaceMode) {
        ...
        if mode == .freezeFrame, current.freezeDuration == nil {
            ...
            analysisWorkspace = AnalysisWorkspaceRequest(mode: mode, clip: frozen, recording: source.recording, seconds: seconds, insertsFreeze: true, parentClipID: current.id)
        } else {
            analysisWorkspace = AnalysisWorkspaceRequest(mode: current.freezeDuration == nil ? .video : .freezeFrame, clip: current, recording: source.recording, seconds: source.seconds)
        }
    }
        ...
        ScrollView(.horizontal) {
            HStack(spacing: 8) {
                Button("Edit layer", systemImage: "pencil") {
                    ...
                    playback.pause()
                    analysisWorkspace = .init(mode: clip.freezeDuration == nil ? .video : .freezeFrame, clip: clip, recording: recording, seconds: clip.freezeDuration == nil ? max(clip.startSeconds, mark.start) : clip.startSeconds, selectedAnnotation: mark.id)
                ...
                VStack(alignment: .leading, spacing: 24) {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        TextField("Video title", text: $title)
                            .font(.body).textInputAutocapitalization(.sentences).submitLabel(.done)
...
private struct EditorPlayerSurface: UIViewRepresentable {
    let player: AVPlayer
    let fillsFrame: Bool
    func makeUIView(context: Context) -> EditorPlayerView {
        let view = EditorPlayerView()
        view.playerLayer.player = player
        return view
    }
    func updateUIView(_ view: EditorPlayerView, context: Context) {
        if view.playerLayer.player !== player { view.playerLayer.player = player }
        view.playerLayer.videoGravity = fillsFrame ? .resizeAspectFill : .resizeAspect
    }
}

private final class EditorPlayerView: UIView {
...
private struct ClipSourcePicker: View {
    ...
    var body: some View {
        ...
        .navigationTitle("Use footage").navigationBarTitleDisplayMode(.inline)
    }
}
                ...
                Section("Color") {
                    LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 4), spacing: 8) {
                        ForEach(EventColor.allCases) { color in
                            Button { colorHex = color.rawValue; UISelectionFeedbackGenerator().selectionChanged() } label: {
                                HStack(spacing: 5) {
