...
final class VideoThumbnailService {
    ...
    private init() {
        ...
        memoryWarningObserver = NotificationCenter.default.addObserver(
            ...
            MainActor.assumeIsolated { self?.releaseTransientResources() }
        }
    }

    func image(url: URL, seconds: Double, size: CGSize, persistsToDisk: Bool = false, tolerance: Double = 0.2) async -> UIImage? {
        ...
        if let image {
            cache.setObject(image, forKey: key, cost: imageCost(image))
            if let diskURL, let data = image.jpegData(compressionQuality: 0.78) {
