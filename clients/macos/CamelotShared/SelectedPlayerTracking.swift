...
struct PlayerMotion: Codable, Equatable, Sendable {
    ...
    private func stabilisedBox(at index: Int) -> CGRect {
        ...
        guard amount > 0, index > 0, index < samples.count - 1,
              !(correctionTimes?.contains { abs($0 - sample.time) < 1 / 600 } ?? false) else { return sample.box }
...
enum SelectedPlayerTracking {
    static func track(url: URL, seed: CGRect, from start: Double, to requestedEnd: Double,
        ...
        var rejoin = PlayerTrackRejoinConfirmation(start: start)
        ...
        while let sample = output.copyNextSampleBuffer() {
            ...
            let detectionDue = seconds - lastDetection >= (dormant ? 0.4 : neededRecovery || !profile.isConfirmed ? 0.12 : 0.3)
            if neededRecovery, detectionDue, let lastGoodBuffer,
               ...
               let center = camera.point(.init(x: previous.midX, y: previous.maxY)) {
                ...
                // Measure player displacement after aligning the SAME two
                // frames. Subtracting a short pan estimate from a longer screen
                // trajectory invents player motion when the camera accelerates.
                if recoveryPlayerVelocity == nil, let reference = cameraReference,
                   ...
                   let past = pastCamera.point(.init(x: reference.box.midX, y: reference.box.maxY)) {
                    let dt = previousTime - reference.time
                    recoveryPlayerVelocity = CGPoint(x: (previous.midX - past.x) / dt, y: (previous.maxY - past.y) / dt)
                }
                if let velocity = recoveryPlayerVelocity,
                   let moving = trajectory.predicted(at: seconds, playerVelocity: velocity),
            ...
            if detectionDue {
                ...
                func identities(_ boxes: [CGRect]) -> [PlayerIdentityAssociation.Candidate] {
                    boxes.enumerated().map { index, candidate in
                    PlayerIdentityAssociation.Candidate(box: candidate,
                        ...
                        crowded: boxes.enumerated().contains { other, rect in
                            other != index && PlayerTracker.overlap(rect, candidate) > 0.25
                        })
                    }
                }
                var candidates = identities(detected)
                var match = PlayerIdentityAssociation.choose(candidates, expected: predicted,
                ...
                if match == nil, let region = PlayerTrackingSearch.region(around: predicted) {
                    ...
                    match = PlayerIdentityAssociation.choose(candidates, expected: predicted,
                        optical: reliable ? box : nil, profile: profile, recovering: neededRecovery, recoveryAge: recoveryAge)
                }
                if let match {
                    if match.candidate.crowded && !profile.isConfirmed {
                        reliable = false
                                                                 ...
                                                                 requiredObservations: dormant ? 3 : 2, maximumInterval: dormant ? 0.6 : 0.3) {
            ...
            if let prior, rejoin.accept(box, at: seconds, saved: prior) { break }
