...
enum CameraMotionTracking {
    ...
    private static func registerScene(previous: CameraFeatureRegistration.Frame, current: CameraFeatureRegistration.Frame) throws -> CameraTransform? {
    ...
    static func track(url: URL, from start: Double, to end: Double,
        ...
        while let sample = output.copyNextSampleBuffer() {
            ...
            let frameEnd = time + (duration.isFinite && duration > 0 ? duration : 0)
            ...
            guard time - lastTime >= 0.045 || frameEnd >= end - 1 / 600,
            ...
            if let previous {
                ...
                if let anchor, let direct = try? registerScene(previous: anchor, current: current) {
        ...
        if motion.lostAt == nil, let last = motion.samples.last,
           lastFrameEnd >= end - 1 / 600, last.time < end {
