...
enum GameAnnotationEffects {
    static func playerMarker(rect: CGRect, mark: AnalysisAnnotation, time: Double) -> CGPath {
        var height = rect.height, bodyWidth = rect.width
        if let motion = mark.displayPlayerMotion, let body = motion.effectBodyBox(at: time),
           let average = motion.overheadHeight(at: time), body.height > 0 {
            let scale = average / body.height
            height *= scale; bodyWidth *= scale
        }
        let marker = max(6, bodyWidth * 0.24)
        let y = rect.maxY - height - marker * 1.5
        let path = CGMutablePath()
        path.addLines(between: [.init(x: rect.midX - marker, y: y - marker),
                               .init(x: rect.midX + marker, y: y - marker), .init(x: rect.midX, y: y)])
        path.closeSubpath()
        return path
    }
    ...
    static func wall(points: [CGPoint], closed: Bool, mark: AnalysisAnnotation, time: Double, frame: CGRect, context: CGContext, width: CGFloat, projection: GroundEffectProjection? = nil) {
        ...
        func top(_ point: CGPoint) -> CGPoint? {
            if let projection {
                guard let raised = projection.raised(.init(x: (point.x - frame.minX) / frame.width, y: (point.y - frame.minY) / frame.height), meters: mark.wallHeightMeters ?? 2) else { return nil }
                return .init(x: frame.minX + raised.x * frame.width, y: frame.minY + raised.y * frame.height)
            }
        ...
        for (a, b) in edges {
            ...
            guard let upA = top(a), let upB = top(b) else { continue }
    ...
    static func player(rect: CGRect, mark: AnalysisAnnotation, time: Double, context: CGContext, width: CGFloat) {
        ...
        let triangle = playerMarker(rect: rect, mark: mark, time: time)
    ...
    /// shadow, and elevated roof edges. Grounded roofs use the estimated metric
    /// projection; ungrounded roofs retain the stylised screen-space curve.
    static func aerial(points: [CGPoint], mark: AnalysisAnnotation, time: Double, frame: CGRect, context: CGContext, width: CGFloat, projection: GroundEffectProjection? = nil) {
        guard points.count >= 3 else { return }
        ...
        func raised(_ point: CGPoint) -> CGPoint? {
            if let projection {
                guard let top = projection.raised(.init(x: (point.x - frame.minX) / frame.width, y: (point.y - frame.minY) / frame.height), meters: mark.wallHeightMeters ?? 2) else { return nil }
                return .init(x: frame.minX + top.x * frame.width, y: frame.minY + top.y * frame.height)
            }
            return .init(x: point.x, y: point.y - height * 0.65)
        }
        ...
        for (a, b) in zip(vertices, Array(vertices.dropFirst()) + [vertices[0]]) {
            guard let upA = raised(a), let upB = raised(b) else { continue }
            let path = CGMutablePath(); path.move(to: upA)
            if projection != nil { path.addLine(to: upB) }
            else { path.addQuadCurve(to: upB, control: CGPoint(x: (upA.x + upB.x) / 2, y: min(upA.y, upB.y) - height * 0.18)) }
            context.addPath(path); context.strokePath()
        }
        context.restoreGState()
    }
