...
/// One shared time axis, with names inside the clips and a separate track for every
    ...
    var body: some View {
        ...
        ZStack(alignment: .leading) {
            ...
            RoundedRectangle(cornerRadius: 5).fill(tint.opacity(mark.isHidden == true ? 0.10 : selected ? 0.36 : 0.18))
                ...
                .overlay(alignment: .topLeading) {
                    Text(mark.title)
                        .font(.system(size: 10, weight: .semibold)).foregroundStyle(tint).lineLimit(1)
                        .padding(.leading, max(12, -x + 12)).padding(.trailing, 10).padding(.top, 3)
                }
