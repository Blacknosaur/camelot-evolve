...
enum PlayerTrackingLimits {
    /// Maximum trajectory extrapolation horizon. After this, sparse detection
    /// continues looking for a return, without extending the predicted path.
...
enum PlayerIdentityAssociation {
    ...
    static func choose(_ candidates: [Candidate], expected: CGRect, optical: CGRect?, profile: PlayerJerseyProfile,
                       recovering: Bool, recoveryAge: Double = 0) -> Match? {
        ...
        guard let best = ranked.first, best.score >= (recovering ? 0.58 : 0.45),
              ranked.count == 1 || best.score - ranked[1].score >= (dormant ? 0.16 : recovering ? 0.12 : 0.08) else { return nil }
        return best
    }
}
...
struct PlayerRecoveryConfirmation {
    ...
    mutating func accept(_ box: CGRect?, at time: Double, camera: CameraTransform? = nil,
        ...
        guard let box else { pending = nil; confirmations = 0; return false }
        let previous = pending
        pending = .init(time: time, box: box, camera: camera)
        guard let previous, time - previous.time >= 0.04, time - previous.time <= maximumInterval else {
        ...
        confirmations = confirmed ? confirmations + 1 : 1
        if confirmations >= requiredObservations { pending = nil; confirmations = 0; return true }
        return false
    }
}
