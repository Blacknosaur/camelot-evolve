...
enum AnalysisEngine {
    /// Sample motion at 6 fps; interpolation supplies the positions in between.
    static let targetFrameRate = 6.0
    /// Frames are decoded at most this wide; Vision resamples internally anyway.
    static let maximumDecodeWidth = 960.0
    /// Body-pose joints are not used by any editor effect, and the request is by
    /// far the most expensive step on macOS, so it is off unless requested.
    static let usesBodyPose = false
    ...
    static func analyze(url: URL, range: ClosedRange<Double>, progress: @escaping @Sendable (Double) -> Void) async throws -> AnalysisResult {
        ...
        var posesAvailable = Self.usesBodyPose
        var index = 0
        ...
        analysisLog.info("Analysed \(frames.count) frames, \(clustered.tracks.count) tracks in \(range.lowerBound, format: .fixed(precision: 1))–\(range.upperBound, format: .fixed(precision: 1))s in \(Date().timeIntervalSince(started), format: .fixed(precision: 2))s")
...
final class SportsPlayerDetector {
    ...
    func playerBoxes(in buffer: CVPixelBuffer, orientation: CGImagePropertyOrientation, region: CGRect? = nil) throws -> [CGRect] {
        ...
        // Perform every crop in one pass so Vision can schedule them together.
        let requests = regions.map { region -> VNCoreMLRequest in
        ...
        for (region, request) in zip(regions, requests) {
...
private struct SportsDetectorDecoder {
    ...
    private func value(_ array: MLMultiArray, _ candidate: Int, _ component: Int) -> Float {
        // Multi-index access respects the array's strides and element type and
        // bounds-checks, unlike a flat pointer offset computed by hand.
        let indices = [NSNumber(value: 0), NSNumber(value: candidate), NSNumber(value: component)]
        guard indices.indices.allSatisfy({ array.shape[$0].intValue > indices[$0].intValue }) else { return 0 }
