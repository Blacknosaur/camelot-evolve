...
struct AnalysisWorkspaceView: View {
    ...
    init(request: AnalysisWorkspaceRequest, session: AnalysisSession, save: @escaping (CompositionClip) throws -> Void, onClose: (() -> Void)? = nil) {
        self.request = request; self.session = session; self.save = save; self.onClose = onClose
    ...
    var body: some View {
        ...
        .task { await prepare() }
        .onDisappear {
            // Field calibration lives in its own window now, so the workspace
            // only stops when the analysis window itself closes.
            playback.stop(); session.cancel(); trackingTask?.cancel()
        }
    ...
    private var workspaceHeader: some View {
        HStack(spacing: 2) {
            Button("Cancel", systemImage: "xmark") { close() }.labelStyle(.iconOnly)
            ...
            Button("Save") {
                do { try save(clip); close() } catch { self.error = error.localizedDescription }
        ...
        return ZStack(alignment: .topLeading) {
            ...
            Group {
                if clip.freezeDuration != nil {
                    if let still { Image(decorative: still, scale: 1).resizable().scaledToFit() }
            ...
            Menu {
                ...
                if clip.groundCalibration != nil {
                    Button("New ground reference here", systemImage: "ruler.fill") {
                        playback.pause()
                        openCalibration(.init(sourceTime: clip.freezeDuration == nil ? time : clip.startSeconds,
                ...
                Label("Clip tracks", systemImage: "point.3.connected.trianglepath.dotted")
                    .labelStyle(.iconOnly).font(.system(size: 13, weight: .semibold))
                    .frame(width: 44, height: 44).contentShape(.rect)
            }.buttonStyle(.plain)
                .accessibilityIdentifier("analysis-clip-tracks")
        }.buttonStyle(AnalysisTransportStyle()).padding(.horizontal, 4)
            ...
            VStack(spacing: 6) {
                ...
                if selected.motionMode == .keyframes {
                    ...
                    HStack {
                        ...
                        Spacer(minLength: 0)
                        if selected.linkedPlayers != nil, !correctingPlayer {
                            Text("Tap a player above to correct").font(.caption2).foregroundStyle(.secondary)
                        } else {
    ...
    private func openMeasurements() {
        ...
        openCalibration(groundRequest!)
    }

    /// Present field calibration in its own resizable window.
    private func openCalibration(_ calibration: GroundCalibrationRequest) {
        windows.calibration = CalibrationWindowLaunch(url: request.recording.fileURL, request: calibration, apply: applyGroundCalibration)
        openWindow(id: AppWindowID.calibration)
    }
...
private struct AnnotationDrawingSurface: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> DrawingView { DrawingView() }
    func updateNSView(_ view: DrawingView, context: Context) { view.content = self; view.needsDisplay = true }
    final class DrawingView: NSView {
        ...
        override func draw(_ rect: CGRect) {
            ...
            for detection in content.detections {
                ...
                context.setStrokeColor(NSColor.white.withAlphaComponent(0.65).cgColor); context.stroke(mapped)
            }
            if let box = content.selectedPlayer {
                let mapped = CGRect(x: content.frame.minX + box.minX * content.frame.width, y: content.frame.minY + box.minY * content.frame.height, width: box.width * content.frame.width, height: box.height * content.frame.height)
                context.setStrokeColor(NSColor(Theme.signal).cgColor); context.setLineWidth(2); context.stroke(mapped.insetBy(dx: -3, dy: -3))
            }
            if let selected = content.marks.first(where: { $0.id == content.selectedID }), selected.isLocked != true,
               selected.isHidden != true, selected.isActiveInEditor(at: content.time) {
                if selected.tool == .zoom {
                    ...
                    context.setStrokeColor(NSColor(Theme.signal).cgColor); context.setLineWidth(1.5)
                    context.setLineDash(phase: 0, lengths: [6, 4]); context.stroke(crop); context.setLineDash(phase: 0, lengths: [])
                }
                context.setFillColor(NSColor.white.cgColor)
                context.setStrokeColor(NSColor.black.cgColor); context.setLineWidth(2)
                for point in selected.linkedPlayers == nil ? selected.editHandles(at: content.time) : [] {
...
private struct AnalysisPlayerSurface: NSViewRepresentable {
    let player: AVPlayer
    func makeNSView(context: Context) -> Surface { let view = Surface(); view.wantsLayer = true; view.layerPlayer.videoGravity = .resizeAspect; return view }
    func updateNSView(_ view: Surface, context: Context) { view.layerPlayer.player = player }
    static func dismantleNSView(_ view: Surface, coordinator: ()) { view.layerPlayer.player = nil }
