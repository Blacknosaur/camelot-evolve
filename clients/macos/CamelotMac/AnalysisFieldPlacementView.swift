...
struct AnalysisFieldPlacementView: View {
    ...
    init(url: URL, request: AnalysisFieldPlacementRequest, apply: @escaping (AnalysisFieldLayout, [CGPoint]) -> Void, onClose: (() -> Void)? = nil) {
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 8) {
                ...
                Text(points.count < 4 ? "Place \(activePoint + 1) · \(names[activePoint].lowercased()) corner" : "Drag a corner to refine · pinch or scroll to zoom or pan")
                ...
                HStack(spacing: 20) {
                    ...
                    if let image, let position = activePosition {
                        FieldPlacementLoupe(image: image, size: imageSize, point: position).frame(width: 112, height: 100)
                    } else {
                ...
                .navigationTitle("Place visible field")
                .toolbar {
                    ToolbarItem(placement: .cancellationAction) { Button("Cancel") { close() }.accessibilityIdentifier("field-placement-cancel") }
                    ToolbarItem(placement: .confirmationAction) {
                        Button("Apply") { apply(layout, points); close() }.disabled(!valid || image == nil)
    ...
    private var preview: some View {
        GeometryReader { geometry in
            ...
            ZStack(alignment: .topLeading) {
                Color(white: 0.10)
                if let image {
                    Image(decorative: image, scale: 1).resizable().frame(width: frame.width, height: frame.height).position(x: frame.midX, y: frame.midY)
                ...
                .overlay {
                    if let image, let finger, let position = activePosition {
                        FieldPlacementLoupe(image: image, size: imageSize, point: position,
                                            magnification: max(1.4, frame.width / imageSize.width * 2))
    ...
    private func nudge(_ title: String, symbol: String, x: CGFloat, y: CGFloat) -> some View {
        Button(title, systemImage: symbol) {
            guard let point = activePosition, image != nil else { return }
            setPoint(CGPoint(x: point.x + x / imageSize.width, y: point.y + y / imageSize.height))
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            Color(white: 0.1).overlay(alignment: .topLeading) {
                Image(decorative: image, scale: 1).resizable().frame(width: size.width * scale, height: size.height * scale)
                    .offset(x: geometry.size.width / 2 - point.x * size.width * scale,
                            y: geometry.size.height / 2 - point.y * size.height * scale)
            }.clipped()
                .overlay {
